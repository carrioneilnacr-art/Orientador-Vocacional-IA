...
async function main() {
  console.log('=== APLICANDO MIGRACIÓN 010_seed_all_ucsur_careers.sql ===');
  
  const sqlFilePath = path.join(process.cwd(), 'docs', 'architecture', '010_seed_all_ucsur_careers.sql');
  ...
  const [inst] = await sql`SELECT id, name, short_name, website_url FROM institutions WHERE short_name = 'UCSUR'`;
  ...
  console.log(`  -> Carreras únicas con ofertas UCSUR: ${uniqueCareers.size}`);
  ...
  const [coursesCount] = await sql`
    ...
    WHERE camp.institution_id = ${inst.id}
  `;
  console.log(`Cursos registrados en mallas UCSUR: ${coursesCount.count}`);
