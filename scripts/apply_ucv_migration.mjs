...
async function main() {
  const sqlFile = 'docs/architecture/008_seed_all_ucv_careers.sql';
  ...
  console.log('Executing UCV migration...');
  const t0 = Date.now();
  try {
    await sql.unsafe(sqlContent);
    console.log(`✅ Migración UCV ejecutada con éxito en ${Date.now() - t0}ms!`);
