...
async function main() {
  const sqlFile = 'docs/architecture/009_seed_all_uch_careers.sql';
  ...
  console.log('Executing UCH migration...');
  const t0 = Date.now();
  try {
    await sql.unsafe(sqlContent);
    console.log(`✅ Migración UCH ejecutada con éxito en ${Date.now() - t0}ms!`);
